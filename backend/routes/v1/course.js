...
router
  .route("/")
  .post(
    multer({ storage: multerStorage.courseStorage, limits: { fileSize: 1000000000 } }).single(
      "cover"
    ),
...
router
  .route("/:id/sessions")
  .post(
    multer({ storage: multerStorage.sessionsStorage, limits: { fileSize: 1000000000 } }).single(
      "video"
    ),
    authenticatedMiddleware,
    isAdminMiddleware,
