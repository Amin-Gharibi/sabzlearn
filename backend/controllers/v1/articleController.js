...
exports.edit = async (req, res, next) => {
  try {
    const { title, description, body, shortName, categoryID, publish } = req.body;

    const oldArticle = await articleModel.findByIdAndUpdate(
      { _id: req.params.id},
      {
        title,
        description,
    ...
    if (!oldArticle) {
      return res.status(404).json({ message: "Article Not Update!" });
    }

    const newArticle = await articleModel.findById(oldArticle._id).populate("creator", "-password");

    if (oldArticle.cover !== newArticle.cover) {
      const imgPath = path.join(
        __dirname,
        "..",
        "..",
