...
exports.register = async (req, res, next) => {
  try {
    ...
    const user = await userModel.create({
      ...
      profile: Boolean(req.file?.filename) === true ? req.file.filename : 'default_profile_picture.png'
    });
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (isUserBan.length) {
      return res.status(403).json("this phone number banned!");
    }
...
exports.getMe = async (req, res, next) => {
  try {
    const isUserBan = await banUserModel.find({ phone: req.user.phone });
    if (isUserBan.length) {
      return res.status(403).json({
        message: "this phone number banned!",
      });
