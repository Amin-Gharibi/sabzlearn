import {
    ...
    filterArticles, getAllCategories, getSearchParam
} from "./utils/utils.js";
...
const fetchData = async () => {
    try {
        const [{user, headerMenus}, popularCourses, allCategories] = await Promise.all([
            sharedFetches(),
            footerFetches(),
            getAllCategories()
        ])

        await renderPage(user, headerMenus, popularCourses, allCategories)
    } catch (err) {
        console.log(err)
    }
}

const renderPage = async (user, headerMenus, popularCourses, allCategories) => {
    const container = document.querySelector('#content')
    let pageTitle = 'وبلاگ'
    // set page title handling
    if (getSearchParam('cat')) {
        // change page title
        pageTitle = (allCategories.find(category => category.name === getSearchParam('cat'))?.title || getSearchParam('cat'))
    }
    ...
    container.innerHTML = `
        ...
        <div class="container">
            ...
            <div class="flex justify-center items-center gap-x-3 sm:gap-x-5 mt-[60px] text-black dark:text-white font-morabbaBold text-4xl sm:text-6xl whitespace-nowrap">
                <span class="w-7 sm:w-10 h-2.5 bg-amber-400 dark:bg-yellow-400 shrink-0 rounded-sm"></span>
            ...
            <div class="grid items-start grid-rows-1 lg:grid-cols-3 xl:grid-cols-4 gap-3.5 sm:gap-5 mt-9 sm:mt-[100px]">
                ...
                <aside class="lg:sticky top-5">
                    ...
                    <span class="side-bar--titles before:bg-primary relative flex justify-start items-center mb-5 pr-4 font-danaDemiBold text-2xl dark:text-white">
                        ...
                        <ul class="flex flex-col gap-y-4 text-sm text-zinc-700 dark:text-white">
                                ${allCategories.length && allCategories.map(category => {
                                    return `
                                        <li class="flex justify-start items-center gap-2">
                                            ...
                                            <a href="search-categories-articles.html?cat=${category.name}" class="text-zinc-700 dark:text-white text-sm">${category.title}</a>
                                        </li>
                                    `
    }).join('')}
                        </ul>
                ...
                <div class="col-span-1 lg:col-span-2 xl:col-span-3 order-1 lg:order-2">
                    ...
                    <div class="relative h-[68px] hidden sm:flex justify-center lg:justify-start items-center gap-4 mb-5 p-7 bg-white dark:bg-darkGray-800 text-sm dark:border border-darkGray-700 shadow-light dark:shadow-none rounded-2xl overflow-hidden">
                        ...
                        <div class="sorting-data">
                            ...
                            <button class="py-2.5 px-4 text-slate-500 rounded-[8px]" data-value="newest">
                                جدید ترین
                            </button>
                            <button class="py-2.5 px-4 text-slate-500 rounded-[8px]" data-value="oldest">
                                قدیمی ترین
                            </button>
                        </div>
                    </div>
                    <!--filtered articles-->
                    <div class="filtered-articles--wrapper grid grid-rows-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
        ...
        <div class="bottom-sheet">
            ...
            <div class="sorting-data mobile-sorting-data px-6 bg-white dark:bg-darkGray-800 divide-y-[1px] dark:divide-darkSlate">
                ...
                <button class="bottom-sheet__item w-full" data-value="newest">
                <span>
                    جدید ترین
                </span>
                    <svg class="w-5 h-5">
                ...
                <button class="bottom-sheet__item w-full" data-value="oldest">
                <span>
                    قدیمی ترین
                </span>
                    <svg class="w-5 h-5">
